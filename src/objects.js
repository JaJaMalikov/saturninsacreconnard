...
export function initObjects(svgElement, pantinRootId, timeline, memberList, onUpdate, onSave) {
  ...
  function attach(id, memberId) {
    ...
    const el = svgElement.getElementById(id);
    if (!el) return;
    const globalMatrix = el.getCTM();
    if (memberId) {
      const seg = pantinRoot.querySelector(`#${memberId}`);
      if (seg) {
        const local = seg.getCTM().inverse().multiply(globalMatrix);
        Object.assign(obj, matrixToProps(local));
      }
    } else {
      const rootMatrix = pantinRoot.getCTM();
      const local = rootMatrix.inverse().multiply(globalMatrix);
      Object.assign(obj, matrixToProps(local));
    }
  ...
  function renderObjects() {
    ...
    Object.keys(currentFrame.objects).forEach(id => {
      ...
      if (obj.attachedTo) {
        const seg = pantinRoot.querySelector(`#${obj.attachedTo}`);
        if (seg) {
          const res = seg.getCTM().multiply(propsToMatrix(obj));
          el.setAttribute('transform', `matrix(${res.a},${res.b},${res.c},${res.d},${res.e},${res.f})`);
        }
      } else {
        let m = svgElement.createSVGMatrix();
        m = m.translate(currentFrame.transform.tx, currentFrame.transform.ty);
        m = m.rotate(currentFrame.transform.rotate);
        m = m.scale(currentFrame.transform.scale);
        const res = m.multiply(propsToMatrix(obj));
        el.setAttribute('transform', `matrix(${res.a},${res.b},${res.c},${res.d},${res.e},${res.f})`);
      }

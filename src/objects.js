...
export function initObjects(svgElement, pantinRootId, timeline, memberList, onUpdate, onSave) {
  ...
  function setLayer(id, layer) {
    ...
    const el = svgElement.getElementById(id);
    if (!el) return;
    const parent = layer === 'front' ? frontLayer : backLayer;
  ...
  function attach(id, memberId) {
    ...
    if (memberId) {
      ...
      if (seg && el) {
        ...
        const objMatrix = el.getCTM();
        const localMatrix = segMatrix.inverse().multiply(objMatrix);
        const scale = Math.hypot(localMatrix.a, localMatrix.b);
        const rotate = Math.atan2(localMatrix.b, localMatrix.a) * 180 / Math.PI;
        obj.x = localMatrix.e;
        obj.y = localMatrix.f;
        obj.scale = scale;
        obj.rotate = rotate;
      ...
      if (seg && el) {
        ...
        const localMatrix = new DOMMatrix()
          ...
          .rotate(obj.rotate, obj.width / 2, obj.height / 2)
          .scale(obj.scale);
        const globalMatrix = segMatrix.multiply(localMatrix);
        const pantinMatrix = pantinRoot.getCTM();
        const stageMatrix = pantinMatrix.inverse().multiply(globalMatrix);
        const scale = Math.hypot(stageMatrix.a, stageMatrix.b);
        const rotate = Math.atan2(stageMatrix.b, stageMatrix.a) * 180 / Math.PI;
        obj.x = stageMatrix.e;
        obj.y = stageMatrix.f;
        obj.scale = scale;
        obj.rotate = rotate;
      ...
      if (obj.attachedTo) {
        ...
        if (seg) {
          ...
          const localMatrix = new DOMMatrix()
            ...
            .rotate(obj.rotate, obj.width / 2, obj.height / 2)
            .scale(obj.scale);
          const m = segMatrix.multiply(localMatrix);
          const { a, b, c, d, e, f } = m;

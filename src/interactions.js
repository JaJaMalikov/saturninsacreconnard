...
const PANTIN_STATE_KEY = 'pantin-global-transform';
const pantinState = {
  rootGroup: null,
  svgElement: null,
};
...
export function setupPantinGlobalInteractions(svgElement, options, timeline, onUpdate, onEnd) {
  logger.log("setupPantinGlobalInteractions called.");
  ...
  logger.log('Grab element:', grabEl);

  if (!pantinState.rootGroup || !grabEl) {
    logger.warn("Missing elements for global pantin interactions.", {pantinRoot: pantinState.rootGroup, grabEl});
    return;
  }
  ...
  grabEl.addEventListener('pointerdown', e => {
    logger.log("Global drag: pointerdown");
    dragging = true;
  ...
  svgElement.addEventListener('pointermove', e => {
    if (!dragging) return;
    logger.log("Global drag: pointermove");
  ...
  const endDrag = e => {
    if (!dragging) return;
    logger.log("Global drag: pointerup/pointerleave");
    dragging = false;
    grabEl.style.cursor = 'move';
...
export function setupInteractions(svgElement, memberList, pivots, timeline, onUpdate, onEnd) {
  logger.log("setupInteractions (members) called.");
  ...
  memberList.forEach(id => {
    ...
    if (!seg) {
      logger.warn(`Segment ${id} not found.`);
      return;
    }
    ...
    seg.addEventListener('pointerdown', e => {
      logger.log(`Member ${id}: pointerdown`);
    ...
    function onMove(e) {
      if (!rotating) return;
      logger.log(`Member ${id}: pointermove`);
    ...
    function onUp(e) {
      logger.log(`Member ${id}: pointerup/pointerleave`);
      rotating = false;
      seg.style.cursor = 'grab';
